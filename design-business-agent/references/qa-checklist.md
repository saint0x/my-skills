...
Aegis is the studio's own real browser, not just a research tool — run the finished page through it before calling a project done, the same runtime used for research browsing. CUA (`cua` on PATH, source at `~/Desktop/cua`; if the command isn't found, rebuild with `cd ~/Desktop/cua && cargo build --release -p cua`) supplies the piece Aegis doesn't: an actual pixel screenshot, since Aegis reads and acts on the DOM but has no screenshot command of its own.
