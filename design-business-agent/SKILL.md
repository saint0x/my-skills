...
- **This skill adds the layers taste-max doesn't own:** the business/research/conversion layer, the generative asset pipeline (Higgsfield/Cling/Nano Banana), video and scroll-frame engineering, and the studio-level QA/shipping gate.
...
2. **Research before designing.** Pull current competitor sites, category leaders, and conversion patterns for this niche. See [references/research-and-strategy.md](references/research-and-strategy.md). Separate evidence from heuristics from trend from taste — don't present one as another.
...
10. **Run the QA gate before calling it done.** Visual, functional, performance, conversion, technical — see [references/qa-checklist.md](references/qa-checklist.md). Test on a representative phone/slow network, not just the dev machine.
